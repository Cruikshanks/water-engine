 ...
 *
 * They are both requests to an app to terminate. Without this plugin the app would immediately stop when the signal is
 * received. Our issue is this means any in-flight requests will be dropped and not allowed to complete and return a
 * response.
 *
 * We want to avoid dropping any existing requests whilst also not accepting new ones. By handling the terminate signals
 * we can call {@link https://hapi.dev/api/?v=20.0.3#-await-serverstopoptions|Hapi's server.stop()} which does exactly
 * that.
 *
 ...
 *
 * @see
 * {@link https://www.gnu.org/software/libc/manual/html_node/Termination-Signals.html|Termination Signals} for further
 * details about `SIGTERM` and `SIGINT`.
 *
 * @module StopPlugin
 */
